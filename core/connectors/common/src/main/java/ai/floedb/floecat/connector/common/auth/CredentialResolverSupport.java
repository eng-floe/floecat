...
public final class CredentialResolverSupport {
  ...
  public static ConnectorConfig apply(ConnectorConfig base, AuthCredentials credential) {
    return apply(base, credential, AuthResolutionContext.empty());
  }

  public static ConnectorConfig apply(
      ConnectorConfig base, AuthCredentials credential, AuthResolutionContext context) {
    if (credential == null) {
      return base;
    }
  ...
  private static String exchangeRfc8693(
      AuthCredentials.TokenExchange exchange,
  ...
  private static String exchangeAzureObo(
      AuthCredentials.TokenExchange exchange,
      Map<String, String> extraParams,
      Map<String, String> headers,
      String subjectToken) {
  ...
  private static String requireSubjectToken(AuthResolutionContext context) {
    if (context == null) {
      throw new IllegalArgumentException("subject_token required for token exchange");
    }
    String token = context.subjectToken();
    if (token == null || token.isBlank()) {
      throw new IllegalArgumentException("subject_token required for token exchange");
    }
    return token;
