 ...
 *
 * <p>The opt-in is required rather than implicit because it decides whether the storage service
 * <em>attempts</em> a source-catalog vend at all. Making it implicit for every Unity connector
 * would turn a genuinely misconfigured storage authority into a vend attempt, and resurface the
 * misconfiguration far away as an opaque FileIO read failure instead of a precise "no authority
 * covers this location".
 *
 * <p>Not the reconciler's absorb gate, which is a different question and never consults this
 * parser: {@code SourceCatalogVending.clientAppliesVendedCredentials} answers {@code false} for
 * {@code DELTA} unconditionally, because this connector's engine builds its S3 client from the
 * connector's own options rather than from anything a catalog hands back at load time. Absorbing
 * for Delta would trade that precise failure for a 403 on the first read whether or not vending was
 * declared. Iceberg REST is where the two gates coincide -- the same invariant {@link
 * IcebergAccessDelegation} documents.
 */
public final class DatabricksAccessDelegation {
   ...
   * authority.
   *
   * <p>An absent {@code delta.source} reads as Unity Catalog because that is {@code
   * DeltaConnectorFactory.selectSource}'s default. Nothing requires the option: a Delta connector
   * created without it is accepted and treated as Unity, so this is the live default rather than a
   * legacy-only fallback. A caller that needs to know whether the source was stated must read the
   * option itself.
   */
  ...
  /**
   * Every value {@link #isRecognizedValue} accepts, as a set rather than a rendering.
   *
   * <p>For round-tripping the two against each other. The rejection message is built from {@link
   * #acceptedValuesDescription}, not from this, so a test that took a display string back apart
   * would be pinning a format nothing else depends on. Two things are accepted and are not in here:
   * omitting the property, and any of these spelled with underscores in place of hyphens.
   */
  public static Set<String> acceptedValues() {
    return Stream.concat(TRUTHY.stream(), FALSY.stream()).collect(Collectors.toUnmodifiableSet());
  }

  /**
   * The same values, grouped by what they do, for an operator reading a rejection.
   *
   * <p>The flat list answers the wrong question. Someone who typed {@code vended_credentials} wants
   * vending on and needs to know which four strings do that; sorting the two sets together
   * interleaves them so the distinction that matters is the one the message hides.
   */
  public static String acceptedValuesDescription() {
    return TRUTHY.stream().sorted().collect(Collectors.joining(", "))
        + " to enable vending, or "
        + FALSY.stream().sorted().collect(Collectors.joining(", "))
        + " to disable it (underscores may stand in for hyphens)";
  }
   ...
   * time but makes {@code vended-credential} (singular) or {@code vended-creds} indistinguishable
   * from a deliberate opt-out -- and because both gates that read this parser then agree the
   * connector never opted in, nothing anywhere reports why reads fall back to a storage authority
  ...
  private static boolean isTruthy(String value) {
    ...
    // Underscores fold to hyphens, as IcebergAccessDelegation.headerDeclaresVendedCredentials and
    // TableGatewaySupport.usesVendedCredentials both do -- vended_credentials is what DuckDB sends,
    // not a typo. Rejecting it here would have made the same string valid on the Iceberg header and
    // a create-time failure on this option. vended-credential, singular, is still unrecognized.
    return TRUTHY.contains(value.trim().toLowerCase(Locale.ROOT).replace('_', '-'));
  }
}
