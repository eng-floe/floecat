...
- **Audit log (`log/audit.json`)** – Dedicated handler for RPC-level audit entries. The service routes
  specific categories (catalog, namespace, directory, table, view, account, query lifecycle, connector RPCs)
...
- **JSON format** – Both service and audit logs use Quarkus JSON logging by default
  ...
  `correlation_id`, `account_id`, and `subject`, populated by `InboundContextInterceptor`.
...
- **Auditing** – Inspect `log/audit.json` to review CRUD requests per account with correlation IDs for
  compliance.
