...
- A Caffeine-backed cache keyed by resource ID + pointer version so invalidation is deterministic.
...
`resolveName` first short-circuits when the NameRef embeds a `ResourceId`. Otherwise it queries the
Directory service (table, view) and enforces the same ambiguity/unresolved error codes used by
`DirectoryService.Resolve*`. Graph callers get a consistent NameRef → ResourceId translation that
already mirrors RPC behavior.
...
- Expose metrics counters for cache hits/misses and downstream RPC fallbacks.
