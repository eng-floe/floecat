...
| `SnapshotService` | `ListSnapshots`, `GetSnapshot`, `CreateSnapshot`, `DeleteSnapshot` | Pins upstream checkpoints and timestamps. Finalized snapshots expose a system-owned `reuse_manifest_ref`; `SnapshotSpec` does not accept that field. |
