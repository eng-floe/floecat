...
Every system node exposes a `ResourceId` whose `id` field looks like a UUID but is deterministically computed. The canonical identity string for the object (namespace-qualified name plus any versioned signatures) is hashed with SHA-256, truncated to 16 bytes, forced to carry the fixed two-byte marker `0xABCD`, and then XORed with a catalog-specific mask. The marker distinguishes system-generated IDs from arbitrary UUIDs, while the selected catalog kind remains part of the identity. The generated bytes also obey the RFC-4122 variant 4 layout (version nibble `4`, variant bits `10`), so serializers and tooling can treat them like canonical UUIDs while the mask still exposes a deterministic catalog identity. When an engine is selected, its kind names the system catalog; otherwise the selected environment names it; when neither axis is selected, the catalog is `floecat_internal`. Use `CatalogContext.effectiveSystemCatalogKind()` and `SystemNodeRegistry.systemCatalogContainerId(...)` whenever you need that identifier during builtin graph construction or tests.
