...
Every system node exposes a `ResourceId` whose `id` field looks like a UUID but is deterministically computed. The canonical identity string for the object (namespace-qualified name plus any versioned signatures) is hashed with SHA-256, truncated to 16 bytes, forced to carry the fixed two-byte marker `0xABCD`, and then XORed with an engine-specific mask. The marker distinguishes system-generated IDs from arbitrary UUIDs, while the selected catalog kind remains part of the identity. The generated bytes also obey the RFC-4122 variant 4 layout (version nibble `4`, variant bits `10`), so serializers and tooling can treat them like canonical UUIDs while the engine mask still exposes a deterministic catalog identity. Every selected engine contributes exactly one system catalog container (the catalog whose `engineKind` equals the engine name), so `ResourceKind=RK_CATALOG` always refers to the selected engine catalog container. Use `SystemNodeRegistry.systemCatalogContainerId(engineKind)` whenever you need that identifier during builtin graph construction or tests.
...
Both `ServiceLoaderSystemCatalogProvider` and `FloecatInternalProvider` fail fast on `Severity.ERROR` validation issues during catalog load, so extension authors should treat these defaults as part of the public contract.
...
│ - built with CatalogGraphView/SystemGraph     │
└────────────────────────────────────────────┘
                    ↓
┌────────────────────────────────────────────┐
...
- **`SystemObjectScannerProvider`** – SPI for internal or engine-owned definitions and scanners. `definitions()` lists the definitions for the supplied context. `supports(NameRef, context)` gates when a definition applies. `provide(scannerId, context)` lets the runtime look up a scanner for a FLOECAT node’s `scannerId`. `EngineCatalogProvider` extends this SPI and can implement the live engine path without copying changing engine metadata into PBtxt.
- **`CatalogEnvironmentProvider`** – SPI for environment-owned relations and scanners. Its methods receive the complete `CatalogContext`, so an environment can be selected independently of the engine whose capabilities describe the request.
- **`ServiceLoaderSystemCatalogProvider`** – Discovers engine and environment providers separately, loads optional static engine catalog data, and hands it to `SystemDefinitionRegistry`. It exposes the shared internal provider, engine providers, and environment providers; composition happens later in `SystemNodeRegistry`.
- **`SystemNodeRegistry` + `SystemGraph`** – The registry filters the snapshot for the requested environment/engine context, materialises `GraphNode`s and `SystemTableNode`s, and caches the result. It composes the selected providers into one logical catalog. Identical definitions are deduplicated; conflicting definitions fail during materialization. `SystemGraph` builds `GraphSnapshot`s from those nodes and keeps them in an LRU `LinkedHashMap` keyed by the complete catalog context. Each snapshot stores namespace buckets, table relations, and a `nodesById` map for constant-time resolution.
...
`FloecatInternalProvider` wraps `InformationSchemaProvider` and is selected explicitly as the `floecat_internal` catalog. It is not seeded into explicit engine or environment selections. A request boundary may select it when both axes are absent; otherwise `information_schema` must come from the selected provider composition.
...
1. `ctx.listNamespaces()`/`ctx.listTables()` reuse the graph view, so the catalog’s graph snapshot is only enumerated once per scan.
2. `columns` resolves schemas via `ctx.graph().tableSchema(table.id())`, which delegates through the graph-view/metadata-graph caches so repeated scans avoid redundant schema materialization work.
...
1. **Implement `EngineCatalogProvider`** for engine-owned capabilities and live engine relations. Engine-owned tables use `TABLE_BACKEND_KIND_ENGINE`. Leave `loadSystemCatalog()` empty when engine metadata is live rather than PBtxt-backed.
2. **Implement `CatalogEnvironmentProvider`** for environment-owned relations. Return definitions and scanners from the complete `CatalogContext`; do not duplicate engine capabilities in the environment provider. Environment tables use `TABLE_BACKEND_KIND_FLOECAT` or `TABLE_BACKEND_KIND_STORAGE`.
