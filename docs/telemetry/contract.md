...
| floecat.core.cache.evicted.weight.bytes | COUNTER | bytes | v1 | Retained bytes released by eviction, tagged by cache name. Read with evictions to tell many small entries from few large ones. | cache, component, operation | account, cache, component, operation |
