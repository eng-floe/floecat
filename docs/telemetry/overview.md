...
- **Reconcile job GC observability** – reconcile job GC emits `floecat.core.gc.collections` result values for tick, ready-slice, account-slice, delete, and quarantine work. Unreadable canonical job payloads, dedupe-owned jobs, ready entries, and root summaries are retained and counted with `*-quarantined` result values instead of deleted. The scheduler also exposes service gauges under `floecat.service.gc.reconcile_jobs.*` for last-tick account throughput, current cached account-page position, active continuation-token count, last-tick quarantine count, and last-tick delete count. Each completed tick writes one INFO summary log with the same low-cardinality totals; failures write a WARN and increment `floecat.core.gc.errors` with `gc=reconcile-jobs`.
