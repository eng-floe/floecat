...
- **Scheduler-driven gauges** – the GC schedulers (`floecat.service.gc.*`) and storage refresher expose gauges that update when those schedulers run. The GC metrics (`enabled`, `running`, `last.tick.start.ms`, `last.tick.end.ms`) reflect the most recent tick and remain unchanged when the scheduler is disabled (`enabled=0`, `running=0`). Per-account storage pointer and byte counters are maintained incrementally on writes and deletes; the storage refresh controlled by `floecat.metrics.storage.refresh` (default `30s`) only publishes those counters as `floecat.service.storage.account.*` gauges.
...
- **Counter-backed refresh** (e.g., storage account pointers and bytes) publishes incrementally maintained durable counters on a schedule without rescanning pointers or issuing object-store metadata requests.
