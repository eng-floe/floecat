...
public class DeltaIcebergMetadataTranslator {
  ...
  public IcebergMetadata translate(Table table, List<Snapshot> snapshots) {
    ...
    PartitionSpecInfo spec = currentPartitionSpec(current);
  ...
  private PartitionSpecInfo currentPartitionSpec(Snapshot snapshot) {
    if (snapshot != null && snapshot.hasPartitionSpec()) {
      PartitionSpecInfo currentSpec = snapshot.getPartitionSpec();
      if (currentSpec.getSpecId() >= 0) {
        return currentSpec;
      }
    }
    return PartitionSpecInfo.newBuilder().setSpecId(0).build();
  }
  ...
  private NormalizedSchema normalizeSchema(String rawSchemaJson, int desiredSchemaId) {
    try {
      ...
      int schemaId = nonNegativeInt(root.get("schema-id"), desiredSchemaId);
  ...
  private int maxFieldIdInSchema(Object rawFields) {
    ...
    for (Object value : list) {
      if (value instanceof Map<?, ?> map) {
        int id = positiveInt(map.get("id"), 0);
        max = Math.max(max, id);
        Object nestedType = map.get("type");
        if (nestedType instanceof Map<?, ?> nestedMap) {
          max = Math.max(max, maxFieldIdInNested(nestedMap));
        }
      }
    }
    return max;
  ...
  private int maxFieldIdInNested(Map<?, ?> type) {
    int max = 0;
    Object rawFields = type.get("fields");
    if (rawFields instanceof List<?> list) {
      max = Math.max(max, maxFieldIdInSchema(list));
    }
    for (String key : List.of("elementType", "keyType", "valueType", "element", "key", "value")) {
      Object nested = type.get(key);
  ...
  private static final class IdAllocator {
    ...
    private IdAllocator(int initialNextId) {
      this.nextId = Math.max(1, initialNextId);
    }

    private int assign(int preferred) {
      if (preferred > 0) {
