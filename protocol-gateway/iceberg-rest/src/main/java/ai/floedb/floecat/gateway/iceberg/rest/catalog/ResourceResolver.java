...
public class ResourceResolver {
  ...
  public NamespaceRef namespace(CatalogRef catalog, String namespace) {
    ...
    try {
      namespaceId = tableSupport.resolveNamespaceId(catalog.catalogName(), namespacePath);
  ...
  public TableRef table(NamespaceRef namespace, String table) {
    ...
    try {
      tableId =
          tableSupport.resolveTableId(namespace.catalogName(), namespace.namespacePath(), table);
    } catch (StatusRuntimeException e) {
      if (e.getStatus().getCode() == Status.Code.NOT_FOUND) {
