...
public class DeltaIcebergMetadataTranslator {
  ...
  public IcebergMetadata translate(Table table, List<Snapshot> snapshots) {
    ...
    String metadataLocation = syntheticLocation(table, Math.max(0L, currentSnapshotId));
    String tableUuid =
  ...
  private String syntheticLocation(Table table, long snapshotId) {
    String tableId =
        table != null && table.hasResourceId() && table.getResourceId().getId() != null
            ? table.getResourceId().getId()
            : "unknown";
    return "floe+delta://" + tableId + "/metadata/" + snapshotId + ".metadata.json";
  }
