...
public class DurableReconcileJobStore implements ReconcileJobStore {
  ...
  private static final int DEFAULT_PROJECTION_REFRESH_CHILD_PAGE_SIZE = 250;
  private static final int DEFAULT_PROJECTION_REFRESH_MAX_PAGES_PER_MAINTENANCE = 8;
  ...
  private boolean refreshProjectedParent(String accountId, String parentJobId) {
    ...
    for (int pagesProcessed = 0;
      ...
      StoredReconcileJob dirtyDescendant =
          page.records().stream()
              .filter(this::hasDirtyDescendantProjection)
              .findFirst()
              .orElse(null);
      if (dirtyDescendant != null) {
        boolean childRefreshCompleted =
            refreshProjectedParent(dirtyDescendant.accountId, dirtyDescendant.jobId);
        StoredReconcileJob refreshedDirtyDescendant =
            loadByAnyAccount(dirtyDescendant.jobId).map(env -> env.record).orElse(dirtyDescendant);
        if (!hasDirtyDescendantProjection(refreshedDirtyDescendant)) {
          ensureProjectionRefreshScheduled(accountId, parentJobId);
          LOG.infof(
              "projection_refresh parentJobId=%s targetGeneration=%d continuation=%s"
                  ...
                  + " projectionPublished=false childRefreshCompleted=%s childUnblocked=%s"
                  + " blockedOnChild=%s",
              parentJobId,
              requestedGeneration,
              ...
              cursorRestarted,
              childRefreshCompleted,
              true,
              dirtyDescendant.jobId);
          continue;
        }
        ensureProjectionRefreshScheduled(dirtyDescendant.accountId, dirtyDescendant.jobId);
        ensureProjectionRefreshScheduled(accountId, parentJobId);
        if (cursorRestarted) {
      ...
      var acceptedProjection = projections().upsert(nextProjection);
      boolean projectionAccepted =
          acceptedProjection != null
              && acceptedProjection.appliedGeneration() == requestedGeneration
              && acceptedProjection.equals(nextProjection);
      boolean newerGenerationPresent =
          acceptedProjection != null
              && acceptedProjection.appliedGeneration() > requestedGeneration;
      boolean sameGenerationDifferentPayload =
          acceptedProjection != null
              && acceptedProjection.appliedGeneration() == requestedGeneration
              && !acceptedProjection.equals(nextProjection);
      ...
      LOG.infof(
          "projection_refresh parentJobId=%s targetGeneration=%d continuation=%s"
              + " childrenScanned=%d pageCount=%d hasMore=%s durationMs=%d cursorRestarted=%s"
              + " projectionPublished=%s projectionAccepted=%s"
              + " newerGenerationPresent=%s sameGenerationDifferentPayload=%s",
          parentJobId,
          requestedGeneration,
          nextContinuationPointerKey,
          cursor.childrenScanned(),
          ...
          projectionAccepted,
          projectionAccepted,
          newerGenerationPresent,
          sameGenerationDifferentPayload);
      return completed;
    }
    projectionRefreshCursors().upsert(cursor);
    LOG.infof(
  ...
  private StoredReconcileJob projectedSummaryRecord(StoredReconcileJob record) {
    ...
    if (hasDirtyDescendantProjection(record)) {
      return record;
    }
  ...
  private void ensureProjectionRefreshScheduled(String accountId, String parentJobId) {
    String effectiveAccountId = blankToEmpty(accountId);
    String effectiveParentJobId = blankToEmpty(parentJobId);
    if (effectiveAccountId.isBlank() || effectiveParentJobId.isBlank()) {
      return;
    }
    upsertDirtyParentPointer(effectiveAccountId, effectiveParentJobId);
  }
  ...
  private void upsertDirtyParentPointer(String effectiveAccountId, String effectiveParentJobId) {
  ...
  private boolean hasDirtyDescendantProjection(StoredReconcileJob record) {
    return record != null
