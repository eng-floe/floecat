...
public class ReconcileJobMaintenanceScheduler {
  ...
  @Scheduled(
      every = "{floecat.reconciler.job-store.maintenance.tick-every:1s}",
  ...
  void tick() {
    long maxTickMillis =
        ConfigProvider.getConfig()
            .getOptionalValue(
                "floecat.reconciler.job-store.maintenance.max-tick-millis", Long.class)
            .orElse(250L);
    try {
      jobs.runMaintenanceOnce(maxTickMillis);
    } catch (RuntimeException e) {
      LOG.warnf(e, "Reconcile job maintenance tick failed");
    }
  }
}
