...
public class ReconcileJobReadyIndexMaintenanceScheduler {
  ...
  @Scheduled(
      every = "{floecat.reconciler.job-store.ready-index-maintenance.tick-every:1m}",
  ...
  void tick() {
    ...
    int maxPages =
        config
            .getOptionalValue(
                "floecat.reconciler.job-store.ready-index-maintenance.max-pages-per-tick",
                Integer.class)
            .orElse(4);
    try {
      jobs.runReadyIndexMaintenanceOnce(maxTickMillis, Math.max(1, maxPages));
