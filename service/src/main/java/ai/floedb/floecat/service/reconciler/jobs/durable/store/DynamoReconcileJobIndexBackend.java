...
public class DynamoReconcileJobIndexBackend implements ReconcileJobIndexBackend {
  ...
  public Optional<JobIndexEntrySnapshot> loadIndexEntry(String pointerKey) {
    ...
    if (lookupKey != null) {
      return loadLookupPointer(JobIndexBackendSupport.currentLookupStorageKey(lookupKey));
    }
  ...
  public ReconcileJobIndexCleanupManifest loadCleanupManifest(String canonicalPointerKey) {
    ...
    return cleanupManifest(response.item());
  }
  ...
  public Optional<JobCleanupSession> beginJobCleanup(
    ...
    ReconcileJobIndexCleanupManifest boundedManifest =
        mergeManifests(cleanupManifest(current), fallback);
    if (!manifestComplete || !validCleanupManifest(boundedManifest)) {
      return Optional.empty();
    }
                ...
                client ->
                    client.updateItem(
                        UpdateItemRequest.builder()
                            ...
                            .build()));
        currentVersion = longAttr(response.attributes(), ATTR_VERSION);
        currentBlob = stringAttr(response.attributes(), JobIndexBackendSupport.ATTR_BLOB_URI);
  ...
  private List<TransactWriteItem> buildPointerCheckAbsent(String pointerKey) {
    ...
    if (lookupKey != null) {
      var storageKey = JobIndexBackendSupport.currentLookupStorageKey(lookupKey);
      return List.of(buildCheckAbsent(storageKey.partitionKey(), storageKey.sortKey()));
    }
    var canonicalKey = JobIndexBackendSupport.parseCanonicalJobKey(pointerKey);
    if (canonicalKey != null) {
      return List.of(
