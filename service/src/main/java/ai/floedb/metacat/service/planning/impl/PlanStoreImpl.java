...
import ai.floedb.metacat.planning.rpc.GetPlanBundleRequest;
import ai.floedb.metacat.planning.rpc.GetPlanBundleResponse;
import ai.floedb.metacat.planning.rpc.PlanStore;
...
public class PlanStoreImpl extends BaseServiceImpl implements PlanStore {
  ...
  public Uni<GetPlanBundleResponse> getPlanBundle(GetPlanBundleRequest request) {
    ...
    return mapFailures(
            run(() -> GetPlanBundleResponse.newBuilder().build()), correlationId())
        .onFailure()
        .invoke(L::fail)
        .onItem()
        .invoke(L::ok);
