...
public class InMemoryBlobStore implements BlobStore {
  private static final class Blob { final byte[] data; final BlobHeader hdr; Blob(byte[] d, BlobHeader h){this.data=d;this.hdr=h;} }
  ...
  @Override public byte[] get(String uri) { return Optional.ofNullable(map.get(uri)).map(b -> b.data).orElseThrow(); }
  @Override public Optional<BlobHeader> head(String uri) { return Optional.ofNullable(map.get(uri)).map(b -> b.hdr); }
}