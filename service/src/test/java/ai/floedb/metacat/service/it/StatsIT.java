...
class StatsIT {
  ...
  void snapshot_stats_create_list_get_and_current() throws Exception {
    ...
    var putTableRespOld = statsMutation.putTableStats(
    ...
    var colIdStats = ColumnStats.newBuilder()
        .setTableId(tblId).setSnapshotId(snapOld)
        .setColumnId("id")
        .setColumnName("id").setLogicalType("int")
        .setNullCount(0)
    ...
    var colTsStats = ColumnStats.newBuilder()
        .setTableId(tblId).setSnapshotId(snapOld)
        .setColumnId("ts")
        .setColumnName("ts").setLogicalType("timestamp")
        .setNullCount(10)
    ...
    statsMutation.putColumnStatsBatch(
        PutColumnStatsBatchRequest.newBuilder()
            .setTableId(tblId).setSnapshotId(snapNew)
            .addColumns(colIdStats.toBuilder()
                .setSnapshotId(snapNew)
    ...
    var notFound = assertThrows(StatusRuntimeException.class, () ->
        statsAccess.getTableStats(GetTableStatsRequest.newBuilder()
            .setTableId(tblId)
            .setSnapshot(SnapshotRef.newBuilder().setSnapshotId(9_999_999L).build()).build()));
