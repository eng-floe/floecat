...
quit")
  ...
  if [ "$profile" = "localstack" ]; then
    ...
    namespace_props_resp=$(gateway_json_request \
      ...
      '{"removals":["compose-smoke.missing"],"updates":{"compose-smoke.owner":"minimal-gateway","compose-smoke.endpoint":"namespace-properties"}}' \
      "smoke-minimal-namespace-properties")
...
quit")
    ...
    assert_contains "$label gateway namespace cli owner property" "$namespace_cli_out" "compose-smoke.owner = minimal-gateway"
    ...
    gateway_json_request "$compose_project" DELETE "/v1/examples/namespaces/iceberg/views/${view_name}" '' "smoke-minimal-view-cleanup-src" >/dev/null 2>&1 || true
    gateway_json_request "$compose_project" DELETE "/v1/examples/namespaces/iceberg/views/${view_renamed_name}" '' "smoke-minimal-view-cleanup-dst" >/dev/null 2>&1 || true
    ...
    register_overwrite_resp=$(gateway_json_request \
      ...
      "smoke-minimal-register-overwrite")
